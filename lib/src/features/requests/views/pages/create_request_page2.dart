...
class CreateRequestPage2 extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _CreateJobSecondPageState();
}

class _CreateJobSecondPageState extends ConsumerState<CreateRequestPage2> {
  ...
  DeliverablesType deliverableType = DeliverablesType.values.first; //"Usage type";
  ...
  void initState() {
    ...
    _usageLengthOptions = VConstants.kUsageLengthOptions.sublist(0, VConstants.kUsageLengthOptions.length - 1);
  ...
  void initWithExistingJobData() {
    ...
    deliverableType = DeliverablesType.licenseTypeByApiValue(_job?.deliverablesType ?? '');
    usageLength = _job?.usageLength?.name ?? VConstants.kUsageLengthOptions.first;
    ...
    if ((_job?.minAge ?? 0) > 0 && (_job?.maxAge ?? 0) > 0) {
      _ageRangeValues = RangeValues(_job?.minAge.toDouble() ?? 0.0, _job?.maxAge.toDouble() ?? 0.0);
    }
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      child: Column(
                        children: [
                          ...
                          NewFormRow(
                            ...
                            onUpdate: (val) async {},
                            fieldValue: _selectedLocation['streetAddress'] ?? '',
                          ),
                          NewFormRow(
                            customValidator: widget.jobType == 'Remote' ? (_) => null : (value) => VValidatorsMixin.isNotEmpty(value, field: "City is required"),
                            fieldTitle: 'City',
                          ...
                          NewFormRow(
                            customValidator: widget.jobType == 'Remote' ? (_) => null : (value) => VValidatorsMixin.isNotEmpty(value, field: "County is required"),
                            fieldTitle: 'County',
                            fieldType: FormFieldTypes.text,
                            controller: state,
                          ...
                          // ),
                          NewFormRow(
                            customValidator: widget.jobType == 'Remote' ? (_) => null : (value) => VValidatorsMixin.isNotEmpty(value, field: "Country is required"),
                            fieldTitle: 'Country',
                            fieldType: FormFieldTypes.text,
                            controller: country,
                          ...
                          // ),
                          NewFormRow(
                            customValidator: widget.jobType == 'Remote' ? (_) => null : (value) => VValidatorsMixin.isNotEmpty(value, field: "Postal Code is required"),
                    ...
                    NewFormRow(
                      ...
                      options: DeliverablesType.values.map((type) => type.simpleName).toList(),
                      fieldTitle: 'Deliverables',
                      fieldValue: deliverableType.simpleName,
                      onUpdate: (val) async {
                        var absVal = val ?? '';

                        deliverableType = DeliverablesType.values.singleWhere((type) => type.simpleName == absVal);
                        setState(() {});
                      },
                    ),
                      ...
                      Column(
                        children: [
                          NewFormRow(
                            ...
                            options: LicenseType.values.map((type) => type.simpleName).toList(),
                            fieldTitle: 'License',
                            fieldValue: usageType.simpleName,
                            onUpdate: (val) async {
                              usageType = LicenseType.values.singleWhere((type) => type.simpleName == val);
                              setState(() {});
                            },
                          ),
                          NewFormRow(
                    ...
                    ExpansionTile(
                      ...
                      trailing: Icon(
                        hasAdvancedRequirements ? Icons.arrow_drop_up_rounded : Icons.arrow_drop_down_rounded,
                        color: Theme.of(context).colorScheme.onSurface,
                        size: 32,
                      ),
                      // iconColor: VmodelColors.primaryColor,
                      title: Text("Advanced Requirements", style: Theme.of(context).textTheme.displayMedium!.copyWith(fontWeight: FontWeight.w600, color: Theme.of(context).primaryColor)),
                      children: [
                        addVerticalSpacing(25),
                        VWidgetsDropDownTextField<Ethnicity>(
                          fieldLabel: "Ethnicity",
                          hintText: "",
                          options: Ethnicity.values, //const ["Black/African", "Asian"],
                          onChanged: (val) {
                            setState(() {
                              ethnicity = val;
                            });
  ...
  Future<void> _onCreateOrUpdate() async {
    if (!_formKey.currentState!.validate()) {
      VWidgetShowResponse.showToast(ResponseEnum.warning, message: "Please fill all required fields");
      return;
    }
    _toggleButtonLoader();
    ...
    final success = await ref.read(createJobNotifierProvider.notifier).createJobRequest(isAdvanced: hasAdvancedRequirements, isRequest: true, requestTo: widget.username);
    ...
    if (success) {
      ...
      SnackBarService().showSnackBar(message: "Request created successfully", context: context);
