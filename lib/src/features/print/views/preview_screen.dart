...
class PreviewScreen extends ConsumerStatefulWidget {
  const PreviewScreen({super.key, this.selectedAlbumType, this.username, this.albumType});
...
class _PreviewScreenState extends ConsumerState<PreviewScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    final tabsNumber = ref.read(printGalleryListProvider(widget.username)).valueOrNull ?? [];
    _tabController = TabController(vsync: this, initialIndex: 0, length: tabsNumber.length);
  ...
  void onImageSelected(PhotoPostModel item) {
    ...
    if (selectedImages.contains(item)) {
      ...
      VWidgetShowResponse.showToast(ResponseEnum.warning, message: "Maximum of $maximumSelectedLength pictures allowed");
    } else {
          ...
          bottom: PreferredSize(
            ...
            child: Align(
              ...
              child: TabBar(
                ...
                unselectedLabelStyle: Theme.of(context).textTheme.displayLarge!.copyWith(fontWeight: FontWeight.w500, fontSize: 14),
                labelStyle: Theme.of(context).textTheme.displayLarge!.copyWith(fontWeight: FontWeight.w600, fontSize: 14),
                isScrollable: true,
              ...
              return Column(
                children: [
                  SizedBox(
                    ...
                    child: TabBarView(
                      controller: _tabController,
                      children: galleries.map((e) => _gridView(height, e)).toList(),
                    ),
                  ),
                            ...
                            children: [
                              ...
                              Expanded(
                                child: GridView.builder(
                                    gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 3, mainAxisSpacing: 6, mainAxisExtent: 80),
                                    ...
                                    itemBuilder: (BuildContext context, int index) {
                                      return InkWell(
                                          ...
                                          child: Container(
                                            decoration: BoxDecoration(
                                                color: VmodelColors.primaryColor,
                                                image: (selectedImages.length > index && selectedImages[index].url.isHttpOkay)
                                                    ? DecorationImage(image: CachedNetworkImageProvider(selectedImages[index].url), fit: BoxFit.cover)
                                                    : null),
  ...
  Widget _gridView(double height, GalleryModel gallery) {
    ...
    return GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 3, mainAxisSpacing: 3, mainAxisExtent: height * 0.252),
          ...
          return InkWell(
            ...
            child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: data.url.isHttpOkay ? DecorationImage(image: CachedNetworkImageProvider(data.url, maxHeight: 300), fit: BoxFit.cover) : null,
                ),
                child: Align(
                  ...
                  child: Container(
                      ...
                      height: 14,
                      width: 14,
                      decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: VmodelColors.primaryColor)),
                      // child: data.selected
