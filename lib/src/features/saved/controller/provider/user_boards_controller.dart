...
final userBoardsSearchProvider = StateProvider.autoDispose<String?>((ref) => '');

final userBoardsTotalNumberProvider = StateProvider<int>((ref) => 0);

final pinnedBoardsProvider = Provider.autoDispose<AsyncValue<List<UserPostBoard>>>((ref) {
...
final userPostBoardsProvider = AsyncNotifierProvider<UserPostBoardsNotifier, List<UserPostBoard>>(UserPostBoardsNotifier.new);

class UserPostBoardsNotifier extends AsyncNotifier<List<UserPostBoard>> {
  ...
  Future<List<UserPostBoard>> getUserSavedBoards({required int pageNumber}) async {
    final searchTerm = ref.read(userBoardsSearchProvider);
    final response = await repo.getUserCreatedBoards(pageNumber: pageNumber, pageCount: _pageCount, search: searchTerm);
    ...
    return response.fold((left) {
      ...
      if (boardData.isNotEmpty) {
        final parsedList = boardData
            .map<UserPostBoard>((e) {
              //print("slko8o id: ${e['id']}");
              return UserPostBoard.fromMap(e);
            })
            .where((element) => element.numberOfPosts != 0)
            .toList();
        logger.f('Boards Length ${parsedList.length}');

        parsedList.sort((a, b) => b.createdAt.compareTo(a.createdAt));

        if (pageNumber > 1) {
          _currentPage = pageNumber;
          state = AsyncValue.data([...(state.valueOrNull ?? []), ...parsedList]);
        }
  ...
  Future<Map<int, bool>> createPostBoardAndAddPost(String title, {required int postId}) async {
    ...
    return response.fold((left) {
      ...
      final saveSuccessful = await savePostToBoard(postId: postId, boardId: board.id);
      if (saveSuccessful) {
        //Refresh state
        ref.invalidateSelf();
      }
  ...
  Future<bool> savePostToBoard({required int postId, int? boardId}) async {
    final response = await repo.savePostToBoard(postId: postId, boardId: boardId);
  ...
  Future<bool> renameUserBoard({required int boardId, required String newTitle}) async {
    final mTitle = newTitle.trim();
    if (mTitle.isEmpty) return false;
    final response = await repo.renameUserBoard(boardId: boardId, title: mTitle);
  ...
  Future<bool> deletePostFromBoard({required int postId, int? boardId}) async {
    final response = await repo.deleteSavedPostFromBoard(postId: postId, boardId: boardId);
  ...
  Future<bool> togglePinnedStatus({required int boardId}) async {
    ...
    final currentBoardState = ref.read(currentSelectedBoardProvider.notifier).currentState;

    if (currentBoardState == null) return false;

    //pinned
    final response = currentBoardState.board.pinned ? await repo.unpinPostBoard(boardId: boardId) : await repo.pinPostBoard(boardId: boardId);
    ...
    return response.fold((left) {
      ...
      //print("cniejcknm $right");
      if (success) {
        final updatedBoard = currentBoardState.board.copyWith(pinned: !currentBoardState.board.pinned, title: '${currentBoardState.board.title}');
        // '${currentBoardState.board.title} ${currentBoardState.board.pinned}');
        ref.read(currentSelectedBoardProvider.notifier).setOrUpdateBoard(currentBoardState.copyWith(board: updatedBoard));
        switch (currentBoardState.source) {
          ...
          case SelectedBoardSource.userCreatd:
            ...
            ref.read(recentlyViewedBoardsProvider.notifier).updateItemState(updatedBoard);
            break;
          default:
            break;
        }
  ...
  Future<bool> setPostBoardCoverImage({required int boardId, String? imageUrl, File? selectedImage}) async {
    ...
    final response = await repo.setBoardCoverImage(boardId: boardId, imageUrl: coverImageUrl!);
    ...
    return response.fold((left) {
      ...
      if (success) {
        ...
        final updatedBoard = currentBoardState!.board.copyWith(coverImageUrl: coverImageUrl);
        ref.read(currentSelectedBoardProvider.notifier).setOrUpdateBoard(currentBoardState.copyWith(board: updatedBoard));
        updateItemState(updatedBoard);
        ref.read(recentlyViewedBoardsProvider.notifier).updateItemState(updatedBoard);
      }
  ...
  Future<List<dynamic>?> _uploadBanner(List<File> images) async {
    ...
    final uploadResult = await FileUploadRepository.instance.uploadFiles(images, uploadEndpoint: VUrls.postMediaUploadUrl, onUploadProgress: (sent, total) {
