...
final connectionProvider = Provider<ConnectionRepository>((ref) => ConnectionsRepository());
...
final searchConnections = FutureProvider<Either<CustomException, List<dynamic>>>((ref) async {
  final searchQuery = ref.read(myConnectionsSearchProvider);
  Either<CustomException, List<dynamic>> searchResults = Right([]);
  //print('[vcc] --->> make call with $searchQuery*');
  // return;
  searchResults = await ref.read(connectionProvider).searchConnections(query: searchQuery);
  // return ref.read(connectionProvider).searchConnections(query: searchQuery);
  return searchResults;
});

final getConnections = FutureProvider<Either<CustomException, List<dynamic>>>((ref) async {
...
final getUserConnections = FutureProvider.autoDispose.family<Either<CustomException, List<dynamic>>, String>((ref, username) async {
  return ref.read(connectionProvider).getUserConnections(username);
});

final connectionGeneralSearchProvider = StateProvider.autoDispose<String?>((ref) => null);

final getSentConnections = FutureProvider.autoDispose<Either<CustomException, List<dynamic>>>((ref) async {
...
final getRecievedConnections = FutureProvider.autoDispose<Either<CustomException, List<dynamic>>>((ref) async {
  final term = ref.watch(connectionGeneralSearchProvider);
  return ref.read(connectionProvider).getRecievedConnectionsRequests(search: term);
});
...
class ConnectionNotifier extends ChangeNotifier {
  ...
  Future<Either<CustomException, Map<String, dynamic>>> deleteConnection(bool accept, int connectionId) async {
    final repository = ref.read(connectionProvider);
    late Either<CustomException, Map<String, dynamic>> response;
