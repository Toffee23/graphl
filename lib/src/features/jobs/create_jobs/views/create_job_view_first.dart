...
class _CreateJobFirstPageState extends ConsumerState<CreateJobFirstPage> {
  ...
  void initState() {
    ...
    if (widget.isEdit) {
      SchedulerBinding.instance.addPostFrameCallback((_) => initJobDetails());
      logger.d('This was called');
    }
  }

  void initJobDetails() {
    // final catt = jsonDecode(_job!.category!) as List;
    final catt = _job!.category;
    //dev.log('lllLLLLL >> #${catt.first}#');
    _titleController.text = _job!.jobTitle;
    _priceController.text = '${_job!.priceValue.round()}';
    _shortDescriptionController.text = _job!.shortDescription;
    _briefTextController.text = _job!.brief ?? '';
    _briefLinkController.text = _job!.briefLink ?? '';
    jobType = _job!.jobType;

    preferredGender = Gender.genderByApiValue(_job!.preferredGender);
    priceType = _job!.priceOption;
    creativeBriefSwitchValue = _job!.hasBrief;
    talentType = _job!.talents.first;
    _isMultipleApplicants = _job!.acceptingMultipleApplicants;
    categoryType = catt;
  ...
  Widget build(BuildContext context) {
    ...
    final calendarChevronIconColor =
          ...
          child: Column(
            ...
            children: [
              ...
              Column(
                children: [
                  VWidgetsDescriptionTextFieldWithTitle(
                    ...
                    hintText:
                        "Write a short overview of who you’re looking for...",
                    endIcon: (_titleController.text.isNotEmpty &&
                            jobType != null)
                        ? InkWell(
                            onTap: () async {
                              ...
                              gen.fold(
                                (p0) => SnackBarService().showSnackBar(
                                    message: p0, context: context),
              ...
              if (categoryType != null) ...[
                NewFormRowForServiceCategory(
                  options:
                      categoryType?.subType.map((x) => x.name).toList() ?? [],
                  ...
                  onUpdate: (val) {
                    setState(() {
                      // dropdownIdentifyValue = val;
                      subCategory = categoryType?.subType
                          .where((x) => x.name == val!)
                          .single;
                    });
                    final temp =
              ...
              NewFormRowForServiceCategory(
                ...
                onUpdate: (val) async {
                  ...
                  setState(() {
                    priceType = ServicePeriod.values
                        .where((x) => x.tileDisplayName == val!)
                        .single;
                  });
                  final temp =
                      ref.read(jobDataProvider.notifier).state?.copyWith(
              ...
              NewFormRowForServiceCategory(
                ...
                onUpdate: (val) async {
                  setState(() {
                    preferredGender =
                        Gender.values.where((x) => x.simpleName == val!).single;
                  });
                  final temp =
                      ref.read(jobDataProvider.notifier).state?.copyWith(
                    ...
                    Column(
                      children: [
                        VWidgetsDescriptionTextFieldWithTitle(
                          ...
                          hintText:
                              "Write in detail what work needs to be done",
                          minLines: 4,
                          maxLength: maxDescLength,
                          onChanged: (p0) {
                            final temp = ref
                        ...
                        Flexible(
                          child: VWidgetsPrimaryButton(
                            onPressed: () async {
                              ...
                              if (file != null) {
                                setState(() =>
                                    pickedBriefFile = file.files.single.xFile);
  ...
  void _onContinue() {
    ...
    if (categoryType == null) {
      VWidgetShowResponse.showToast(ResponseEnum.warning,
          message: 'Please select a job category to proceed');
      return;
    }
    if (_priceController.text.isEmpty) {
      VWidgetShowResponse.showToast(ResponseEnum.warning,
          message: 'A price value is required');
      return;
    }
    final temp = ref.read(jobDataProvider.notifier).state!.copyWith(
    ...
    bool _isEdit = widget.isEdit;
    ref.read(singleJobProvider.notifier).state = _job;
    context.push(
        '${Routes.createJobSecondPage.split("/:").first}/$_isEdit/${jobType}');
  }
