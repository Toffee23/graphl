  ...
  Future<void> savePostComments({
    ...
    try {
      ...
      response.fold((left) {
        ...
        debugPrint('current number of comments:: ${state.value!.length}');
        final newComment = NewPostCommentsModel.fromMap(right);
        print(newComment);
        final currentData = state.value;
        state = AsyncValue.data([newComment, ...?currentData]);
        debugPrint('updated number of comments:: ${state.value!.length}');
      });
    } catch (e) {
      //print(e.toString());
    }
...
final featuredProvider = AsyncNotifierProvider.family
    .autoDispose<FeaturedNotifier, bool, int>(
    FeaturedNotifier.new);

class FeaturedNotifier
    extends AutoDisposeFamilyAsyncNotifier<bool, int> {
  final _repository = PostCommentsRepository.instance;
  @override
  FutureOr<bool> build(int postId) async {
    try {
      final response = await _repository.removeFeatured(postId: postId);
      return response.fold((left) {
        //print("failed to get comment ${left.message}");
        VWidgetShowResponse.showToast(ResponseEnum.failed,
            message:'Unable to remove user from post');
        return false;
      }, (right) {
        VWidgetShowResponse.showToast(ResponseEnum.sucesss,
            message:'You have been removed from this post');
        return true;
      });
    } catch (e) {
      return false;
