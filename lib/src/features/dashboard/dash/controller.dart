...
final isGoToDiscover = StateProvider<bool>((ref) {
  return false;
});

/// returns a boolean value for when an open container is opened in
/// user profile galleries
final openContainerOpenedProvider = StateProvider((ref) => false);

/// provides the context of an opened container in the profile galleries so containers can be popped
final openContainerContextProvider =
    StateProvider<BuildContext?>((ref) => null);

/// Provides the buildcontext for the dashboard this
/// implemented for accessing context when uploading post
final dashboardContextProvider = StateProvider<BuildContext?>((ref) => null);
...
class DashTabProvider extends StateNotifier<int> {
  ...
  initFCM(BuildContext context, Ref ref) async {
    ...
    await authNotifier.updateFCMToken(token: token!);
    if (!mounted) return;
