...
class ProfileRepository {
  ...
  Future<Either<CustomException, Map<String, dynamic>>> getUserProfile({required String username}) async {
    try {
...
//   }
  ...
  Future<Either<CustomException, List<dynamic>>> getUserPosts(String? username) async {
  ...
  Future<Either<CustomException, Map<String, dynamic>>> followUser(String? username) async {
    //print('CreatePostRepo Fetching user posts');
    try {
  ...
  Future<Either<CustomException, Map<String, dynamic>>> unFollowUser(String? username) async {
    //print('CreatePostRepo Fetching user posts');
    try {
      final result = await vBaseServiceInstance.getQuery(
        queryDocument: '''
  ...
  Future<Either<CustomException, Map<String, dynamic>>> toggleFollowingNotification2({
...
mutation toggleFollowingNotification(\$notifyOnCoupon: Boolean!, \$notifyOnJob: Boolean!, \$notifyOnPost: Boolean!, \$username: String!) {
  ...
  Future<Either<CustomException, Map<String, dynamic>>> toggleFollowingNotification({
    ...
    try {
      final result = await vBaseServiceInstance.mutationQuery(mutationDocument: '''
mutation toggleFollowingNotification(\$notifyOnCoupon: Boolean!, \$notifyOnJob: Boolean!, \$notifyOnPost: Boolean!, \$notifyOnService: Boolean!, \$username: String!,) {
  toggleFollowingNotification(username: \$username, notifyOnJob: \$notifyOnJob,  notifyOnPost: \$notifyOnPost, notifyOnCoupon: \$notifyOnCoupon, notifyOnService: \$notifyOnService, ) {
    success
  }
}
        ''', payload: {
