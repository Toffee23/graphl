...
class InterestSelectionDialog extends ConsumerStatefulWidget {
  ...
  const InterestSelectionDialog({super.key, this.isOnboarding = false});
...
class _SelectInterestState extends ConsumerState<InterestSelectionDialog> {
  ...
  void initState() {
    ...
    selectedInterests = authState!.interests ?? [];
    super.initState();
  }
          ...
          children: [
                      ...
                      children: List.generate(interests.length, (index) {
                        return Padding(
                          ...
                          child: VWidgetsInterestsButton(
                              enableButton: selectedInterests.contains(interests[index]),
                              buttonTitle: interests[index],
                              onPressed: () {
                                onItemTap(interests[index]);
                              }),
                        );
                      }),
                    ),
            ...
            Padding(
              ...
              child: VWidgetsPrimaryButton(
                  ...
                  buttonTitle: 'Save and continue (${selectedInterests.length} / 20)',
                  onPressed: () async {
                    await update();
                    // goBack(context);
                  }),
  ...
  void onItemTap(String interest) {
    if (selectedInterests.contains(interest)) {
      selectedInterests.remove(interest);
    } else {
      if (selectedInterests.length >= 20) {
        VWidgetShowResponse.showToast(ResponseEnum.warning, message: "Maximum of 20 intrests can be selected");
        return;
      }
  ...
  Future<void> update() async {
    toggleIsloading();
    await ref.read(appUserProvider.notifier).updateProfile(interests: selectedInterests);
    toggleIsloading();
    ...
    SnackBarService().showSnackBar(message: "Interest successfully Updated", context: context);
  }
}
...
class VWidgetsInterestsButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialButton(
      ...
      color: enableButton == true ? buttonColor ?? Theme.of(context).buttonTheme.colorScheme?.background : Theme.of(context).buttonTheme.colorScheme?.secondary,
