...
import 'package:vmodel/src/features/settings/views/booking_settings/views/create_service_page.dart';
...
class _BookingPricesSettingsPageState extends ConsumerState<BookingPricesSettingsPage> {
  final _textStyle = VModelTypography1.normalTextStyle.copyWith(fontWeight: FontWeight.w600, fontSize: 59.sp);
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return VWidgetsServicesCardWidget(
                ...
                onTap: () {
                  navigateToRoute(context, CreateServicePage(servicePackage: item));
                },
                onLongPress: () {
                  showAnimatedDialog(
                    context: context,
                    child: VWidgetsConfirmationPopUp(
                      popupTitle: "Delete",
                      popupDescription: "Are you sure you want to delete the service",
                      onPressedYes: () async {
                        goBack(context);
                        VLoader.changeLoadingState(true);
                        await ref.read(servicePackagesProvider(null).notifier).deleteService(item.id);
