...
class ResetProvider extends StateNotifier<ResetModel> {
  ...
  forgetPasswordFunction(BuildContext context,
    ...
    makeForgetPasswordRequest.fold((onLeft) {
      ...
      // context.push("/reset-verification-code");
  ...
  //
  verifyOTP(String otp, String email, bool deleteOTP) async {
    dismissKeyboard();
    final verifyOTPRequest =
        await resetRepoInstance.verifyOTP(email, otp, deleteOTP);

    return verifyOTPRequest.fold((onLeft) {
      VWidgetShowResponse.showToast(ResponseEnum.warning,
          message: onLeft.message.toString());
      return false;
    }, (onRight) {
