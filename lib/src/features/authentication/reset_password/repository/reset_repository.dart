...
class ResetRepository {
  ...
  Future<Either<CustomException, Map<String, dynamic>>> verifyOTP(
    String email, String code, bool deleteOTP
  ) async {
    try {
      final result =
          await vBaseServiceInstance.mutationQuery(mutationDocument: '''
        mutation verifyOtpCode(\$code : String!, \$email : String, \$deleteOtp : Boolean){
          verifyOtpCode(code: \$code, email: \$email, deleteOtp: \$deleteOtp) {
            message
          }
         }
        ''', payload: {
            "code": code, "email": email, "deleteOtp": deleteOTP});
