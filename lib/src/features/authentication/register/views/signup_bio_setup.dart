          ...
          children: [
            ...
            Form(
              ...
              child: VWidgetsDescriptionTextFieldWithTitle(
                ...
                hintText: isBusinessAccount ? "Tell us a bit about your business..." : "Tell us a bit about yourself...",
                maxLength: VConstants.maxBioLength,
                showCounter: true,
                maxLines: 10,
                onChanged: (value) {
                  //Todo find better way of enbling button
                  final validation = VValidatorsMixin.isNotEmpty(value, field: 'bio');

                  _enableButton = validation == null ? true : false;
                  setState(() {});
                },
                validator: (String? value) {
                  final fieldName = isBusinessAccount ? "Business Description" : "A little about you";
                  final validation = VValidatorsMixin.isNotEmpty(value, field: fieldName);

                  return validation;
                },
                labelStyle: Theme.of(context).textTheme.displayMedium!.copyWith(fontSize: 14, fontWeight: FontWeight.w600, color: Theme.of(context).primaryColor),
                hintStyle: Theme.of(context).textTheme.displayMedium!.copyWith(fontSize: 14, fontWeight: FontWeight.w500, color: Theme.of(context).primaryColor),
                // controller: ,
              ),
            ),
              ...
              builder: ((context, value, child) {
                return VWidgetsPrimaryButton(
                  ...
                  onPressed: () async {
                    ...
                    await ref.read(appUserProvider.notifier).updateProfile(bio: textController.text.trim());
                    // VLoader.changeLoadingState(false);
                    isShowButtonIndicator.value = false;

                    if (mounted) {
                      context.push("/signup-upload-photo");
                      /*navigateToRoute(
                          context,
                          // isBusinessAccount
                          // ? SignUpWebSiteView()
